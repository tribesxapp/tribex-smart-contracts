...
contract CoinFlipGame is VRFConsumerBaseV2, ConfirmedOwner {
    ...
    mapping(address => PlayerRanking) public playerRanking;
    ...
    function fulfillRandomWords(
        ...
        if ((heads && game.heads) || (!heads && !game.heads)) {
            ...
            playerRanking[game.player].volume += volume;
            playerRanking[game.player].wins += 1;
        } else {
            game.result = 2;
            volume = game.amount;

            playerRanking[game.player].volume += volume;
            playerRanking[game.player].losses += 1;
        }
